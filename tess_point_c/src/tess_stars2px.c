...
double ras[NSEC] = {352.6844, 16.5571, 36.3138, 55.0070, 73.5382,\
                    ...
                    359.2829, 20.0449, 24.0414,77.3449,\
                    133.7631, 80.6709,261.2194,254.9290,\
                    253.5335,255.8590,260.4232,266.0595,\
                    275.6978,292.5709,309.1299,325.8933,\
                    343.8717,5.8776,42.6741,97.9629,\
                    116.8315,135.8409,155.0705,227.0409,\
                    311.9633,260.5165,323.4700,355.7011,\
                    16.7112,41.6581,80.1656,157.205,\
                    180.5352,209.0229,249.1776,297.5431,\
                    335.4613,2.5764,25.0148,45.8709,\
                    66.2663,86.382,106.0743,125.4703,\
                    145.1712,166.4824,192.2167,219.6384,\
                    288.1479,251.5614,269.0677,274.188,\
                    278.1539,281.2282};
double decs[NSEC] = {-64.8531,-54.0160,-44.2590,-36.6420,-31.9349,\
                     ...
                     -64.2357,-54.2315,9.2629,22.2220,\
                     16.6536,78.8333,72.3020,69.9395,\
                     66.8396,63.8557,61.5745,60.3042,\
                     32.2323,34.6859,39.7760,47.1247,\
                     56.2316,66.3525,75.8930,-32.3927,\
                     -35.7542,-42.4981,-51.7243,-16.8742,\
                     -17.1415,-78.8295,-74.8966,-65.7252,\
                     -55.9356,-41.8452,-31.0553,-41.1581,\
                     -51.9138,-62.0323,-68.7051,-68.1075,\
                     -60.7536,-50.5513,-40.1221,-31.2349,\
                     -25.1811,-22.6679,-23.7944,-28.2718,\
                     -35.6981,-45.527,-56.6197,-16.0606,\
                     -23.3419,32.2829,30.5688,61.9047,\
                     62.9604,64.6669};
double rolls[NSEC] = {222.1532,220.4335,213.0384,202.8302,191.0517,\
                      ...
                      226.0975,222.7721,291.2985,274.9979,\
                      254.0304,8.0330,213.9191,247.4882,\
                      276.6424,302.3416,325.7085,347.5089,\
                      5.4938,17.1317,27.7999,37.0453,\
                      43.9159,45.8046,32.5811,175.9167,\
                      163.0201,151.2486,141.9459,254.2704,\
                      285.4229,174.8522,215.8459,225.9414,\
                      223.7615,210.3682,186.6394,215.9362,\
                      212.8651,217.7538,236.0704,263.1873,\
                      279.238,282.7413,279.1045,271.3363,\
                      261.1568,249.8954,238.6947,228.4036,\
                      219.7678,213.9682,213.7675,251.3437,\
                      276.7904,347.6386,359.3655,19.4458,\
                      40.3192,62.6836};
...
double* tess_stars2px_sector(double ra_ab, double dec_ab, int sector){
  double radecroll[3], eulc[3], vstar[3], vcam[3], lng, lat, lngd, latd;
  double rdr[3];
  double xyfp[2], ccdpx[2], fitpx[2];
  double *napx = malloc(sizeof(double) * 2);
...
int main(int argc, char const *argv[]) {
  ...
  //   usage(argv[0],argc);
  // }
  tess_stars2px(239.86225470984, -40.367621063031);
  napx = tess_stars2px_sector(239.86225470984, -40.367621063031, 12);
  fprintf(stdout, "%8.3f %8.3f \n", napx[0], napx[1]);
  napx = tess_stars2px_sector(239.86225470984, -40.367621063031, 13);
  fprintf(stdout, "%8.3f %8.3f \n", napx[0], napx[1]);
  // ra_ab = atof(argv[1]);
  // dec_ab = atof(argv[2]);
